...
**Créer une page** : reprendre le gabarit d'une page voisine du même type, garder `class="parcours"` sur `<body>`, le `line-v6.js` en `defer` après `main-v3.js`, le `data-line="plug"` sur le bouton final, et les mêmes ancres `data-line-flank` que ses semblables (le tableau des recettes est dans CLAUDE.md). Une nouvelle fiche speaker se contente donc de son portrait et de sa nav ancrés, comme les soixante autres.
